...
/* Colors imported with cmdline below
cat /opt/X11/share/X11/rgb.txt | awk '{ if (NF == 4) { printf "\"%s\": \"%02x%02x%02x\",\n", $4, $1, $2, $3 }}' | pbcopy
*/
...
extension UIColor {
    ...
    static func colorBy(name: String, defaultColor: UIColor? = .red) -> UIColor? {
        ...
        if let hex = lookup[name] {
            return UIColor.colorBy(name: hex, defaultColor: defaultColor)
        }
        return defaultColor
    }
